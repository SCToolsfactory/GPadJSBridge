...
namespace GPadJSBridge
{
  ...
  public class JoystickCls : DeviceCls
  {
    ...
    public enum CtrlItems
    {
      X = 0, Y, Z, Xr, Yr, Zr, S1, S2, P1, P2, P3, P4,
    }
    ...
    {
      log.Info( "GPadJSBridge-GetContolItems" );
      ClearContolItems( );

      if ( m_jPanel == null ) return;

      m_ctrls[( int )CtrlItems.X] = m_jPanel.Xe;
      m_ctrls[( int )CtrlItems.Y] = m_jPanel.Ye;
      m_ctrls[( int )CtrlItems.Z] = m_jPanel.Ze;
      m_ctrls[( int )CtrlItems.Xr] = m_jPanel.Xre;
      m_ctrls[( int )CtrlItems.Yr] = m_jPanel.Yre;
      m_ctrls[( int )CtrlItems.Zr] = m_jPanel.Zre;
      m_ctrls[( int )CtrlItems.S1] = m_jPanel.S1e;
      m_ctrls[( int )CtrlItems.S2] = m_jPanel.S2e;

      m_ctrls[( int )CtrlItems.P1] = m_jPanel.H1e;
      m_ctrls[( int )CtrlItems.P2] = m_jPanel.H2e;
      m_ctrls[( int )CtrlItems.P3] = m_jPanel.H3e;
      m_ctrls[( int )CtrlItems.P4] = m_jPanel.H4e;
    }
    ...
    public JoystickCls( Joystick device, Control hwnd, int joystickNum, UC_JoyPanel panel, int tabIndex , long axisMax)
    {
      ...
      try {
        ...
        foreach ( DeviceObjectInstance d in m_device.GetObjects( ) ) {
          ...
          if ( ( 0 != ( d.ObjectId.Flags & DeviceObjectTypeFlags.Axis ) ) ) {
            ...
            m_device.Properties.Range = new InputRange( 0, (int)axisMax );
          }
      ...
      try {
        ...
        if ( ( e.ResultCode == ResultCode.NotAcquired ) || ( e.ResultCode == ResultCode.InputLost ) ) {
          ...
          try {
            ...
            // Failed to acquire the device. This could be because the app doesn't have focus.
            return null;  // EXIT unaquired
          }
        }
        else {
          log.Error( "Unexpected Poll Exception", e );
          return null;  // EXIT see ex code
        }
      }


      // Get the state of the device - retaining the previous state to find the lates change
      try { return m_device.GetCurrentState( ); }
      // Catch any exceptions. None will be handled here, 
      // any device re-aquisition will be handled above.  
      catch ( SharpDXException ) {
        return null;
